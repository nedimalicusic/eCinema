...
import '../models/searchObject/country_search.dart';
...
class CountryProvider extends BaseProvider<Country> {
  ...
  Future<List<Country>> getPaged({CountrySearchObject? searchObject}) async {
    var uri = Uri.parse('$apiUrl/Countries/GetPaged');
    ...
    if (response.statusCode == 200) {
      ...
      return items.map((d) => fromJson(d)).cast<Country>().toList();
    } else {
